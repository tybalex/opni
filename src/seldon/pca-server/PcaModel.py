...
class PcaModel:
    ...
    def get_sentence_vector(self, tokens):
        word_vectors = [self.fasttext_model.wv[word] for word in tokens]
        processed_word_vectors = np.array([div_norm(wv) for wv in word_vectors])
        return np.mean(processed_word_vectors, axis=0)

    def get_window_vec(self, log, window):
        start_time = time.time()
        window = str(window)
        tokenized = [space_tokenize(x) for x in log]
        logging.info(("--- tokenized in %s seconds ---" % (time.time() - start_time)))
        vecs = [
            self.get_sentence_vector(tokens) for tokens in tokenized
        ]  ## this is too slow
        logging.info(("--- get vector in %s seconds ---" % (time.time() - start_time)))
