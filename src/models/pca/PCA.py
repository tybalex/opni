...
class PCA(object):
    ...
    def save(self, filename="pca_model.json"):
        with open(filename, "w") as fout:
            out_dict = {}
            out_dict["proj_C"] = self.proj_C.tolist()
            # out_dict["components"] = self.components
            out_dict["n_components"] = self.n_components
            out_dict["threshold"] = self.threshold
            out_dict["c_alpha"] = self.c_alpha
            json.dump(out_dict, fout)

    def load(self, filename="pca_model.json"):
        with open(filename, "r") as fin:
            out_dict = json.load(fin)
            self.proj_C = np.array(out_dict["proj_C"])
            # self.components = out_dict["components"]
            self.n_components = out_dict["n_components"]
            self.threshold = out_dict["threshold"]
            self.c_alpha = out_dict["c_alpha"]
